...
import { hash_table } from ".";

test("create hash table of certain size", () => {
  const defaultHashTable = hash_table();
  expect(defaultHashTable.buckets.length).toBe(32);

  const biggerHashTable = hash_table(64);
...
test("set, read and delete data with collisions", () => {
  const hashTable = hash_table(3);
...
test("be possible to add objects to hash table", () => {
  const hashTable = hash_table();
...
test("get all the values from empty hash table", () => {
  const hashTable = hash_table();
  expect(hashTable.getValues()).toEqual([]);
});

test("get all the values in case of hash collision", () => {
  const hashTable = hash_table(3);

  // Keys `ab` and `ba` in current implementation result in one hash (one bucket).
